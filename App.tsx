...
const App: React.FC = () => {
  const [journey, setJourney] = useState<"admin" | "student">("student");
  ...
  const handleMarketAction = (action: string, item: string) => {
    if (action === "plan") {
      ...
      setTimeout(
        () =>
          showToast("Correlation found: 94% match with Alumni data", "info"),
        2000,
      );
    }
    showToast(`${action} for ${item}`, "info");
  };
  ...
  const handleAutoDraft = () => {
    ...
    setTimeout(
      () => showToast("Proposal Draft Saved to Workbench", "success"),
      2500,
    );
  };
  ...
  const handleSearch = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === "Enter") {
      showToast(
        `Searching global database for "${e.currentTarget.value}"...`,
        "loading",
      );
  ...
  const renderStudentJourney = () => {
    switch (studentViewState) {
      ...
      case "student":
        return (
          <StudentDashboard
            onBackToWaypoint={() => setStudentViewState("waypoint")}
          />
        );
      default:
        return (
  ...
  const renderAdminContent = () => {
    switch (activeTab) {
      case "Dashboard":
        return (
          <MainDashboard
            onStudentClick={() => setJourney("student")}
            onAdvisorClick={() => {
              /* Logic for advisor journey */
            }}
          />
        );
      ...
      case "Advisor Journey":
        return (
          <AdvisorJourneyDashboard
            onBackToAdmin={() => setActiveTab("Dashboard")}
          />
        );
      case "Curriculum":
        return (
          <CurriculumDashboard onDesignClick={() => setIsWorkshopOpen(true)} />
        );
      case "Reports":
        return <ReportsDashboard onDeepAnalysis={handleDeepAnalysis} />;
      default:
        return <div>Select a tab</div>;
    }
  };

  const renderAdminJourney = () => (
    <main
            ...
            >
              <div className='flex items-center'>
                {isSidebarCollapsed ? (
                  ...
                  <img
                    src='/waypoint.png'
                    alt='Waypoint - An Ascend Careers Platform'
